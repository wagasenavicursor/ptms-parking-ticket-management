...
export type SearchSelectOption = { value: string; label: string; search?: string; highlighted?: boolean; badge?: string };
...
@Component({
  ...
  template: `
    <div class="search-select" [class.open]="open" [class.has-query]="query.trim()" [class.no-toggle]="!showToggle">
      ...
      <div class="search-select-menu" *ngIf="open">
        ...
        <button type="button" class="search-select-option" [class.active]="i===activeIndex" [class.highlighted-option]="option.highlighted" *ngFor="let option of filteredOptions; let i=index" (mousedown)="$event.preventDefault();choose(option)">
          ...
          <span class="option-flags"><span class="option-badge" *ngIf="option.badge">{{option.badge}}</span><span class="option-secondary" *ngIf="splitLabel(option.label).secondary"><ng-container *ngFor="let part of highlighted(splitLabel(option.label).secondary)"><mark *ngIf="part.match">{{part.text}}</mark><span *ngIf="!part.match">{{part.text}}</span></ng-container></span></span>
        </button>
  ...
  `,
  styles: [`
    :host{display:block;position:relative}.search-select{position:relative}.search-select input{width:100%;padding-right:42px}.search-select.no-toggle input{padding-right:12px}.search-select.has-query input{border-color:#f59e0b;background:#fff8e6;box-shadow:0 0 0 3px rgba(245,158,11,.16);font-weight:700;color:#7c2d12}.search-select-toggle{position:absolute;right:5px;top:50%;transform:translateY(-50%);border:0;background:transparent;color:#64748b;font-size:19px;cursor:pointer;padding:6px 10px}.search-select-menu{position:absolute;z-index:1500;left:0;right:0;top:calc(100% + 5px);max-height:280px;overflow:auto;background:#fff;border:1px solid #cbd5e1;border-radius:12px;box-shadow:0 16px 36px rgba(15,23,42,.18);padding:6px}.search-select-option{display:flex;width:100%;align-items:center;justify-content:space-between;gap:14px;text-align:left;border:0;background:#fff;color:#1e293b;padding:10px 12px;border-radius:8px;cursor:pointer;font:inherit}.search-select-option.highlighted-option{border:1px solid #b8d9a8;background:#f3faef}.search-select-option:hover,.search-select-option.active{background:#eaf3ff}.option-main{font-weight:750;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.option-flags{display:flex;align-items:center;justify-content:flex-end;gap:6px;min-width:0}.option-badge{flex:0 0 auto;padding:3px 7px;border-radius:999px;background:#dff1d5;color:#39751c;font-size:9px;font-weight:900;text-transform:uppercase}.option-secondary{flex:0 1 auto;max-width:160px;padding:4px 9px;border:1px solid #bfdbfe;border-radius:999px;background:#eff6ff;color:#1d4ed8;font-size:12px;font-weight:800;letter-spacing:.02em;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.search-select-option:hover .option-secondary,.search-select-option.active .option-secondary{background:#dbeafe;border-color:#93c5fd}.search-select-option mark{background:#fde68a;color:#78350f;border-radius:3px;padding:0 2px;font-weight:800}.option-secondary mark{background:#fcd34d}.search-select-empty{padding:13px;color:#64748b;font-size:13px}.empty-option{display:block;color:#64748b;border-bottom:1px solid #e2e8f0;border-radius:0;margin-bottom:4px}@media(max-width:560px){.search-select-option{align-items:flex-start;flex-direction:column;gap:6px}.option-flags{width:100%;justify-content:flex-start;flex-wrap:wrap}.option-secondary{max-width:100%}}
  `]
})
