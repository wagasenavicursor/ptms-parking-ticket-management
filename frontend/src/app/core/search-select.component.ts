...
@Component({
  ...
  template: `
    <div class="search-select" [class.open]="open" [class.has-query]="query.trim()">
      <input
        type="text"
        role="combobox"
        autocomplete="off"
      ...
      >
      <button type="button" class="search-select-toggle" tabindex="-1" (click)="toggle()" aria-label="Show options">⌄</button>
      <div class="search-select-menu" *ngIf="open">
        ...
        <button type="button" class="search-select-option" [class.active]="i===activeIndex" *ngFor="let option of filteredOptions; let i=index" (mousedown)="$event.preventDefault();choose(option)">
          <ng-container *ngFor="let part of highlighted(option.label)"><mark *ngIf="part.match">{{part.text}}</mark><span *ngIf="!part.match">{{part.text}}</span></ng-container>
        </button>
  ...
  `,
  styles: [`
    :host{display:block;position:relative}.search-select{position:relative}.search-select input{width:100%;padding-right:42px}.search-select.has-query input{border-color:#f59e0b;background:#fff8e6;box-shadow:0 0 0 3px rgba(245,158,11,.16);font-weight:700;color:#7c2d12}.search-select-toggle{position:absolute;right:5px;top:50%;transform:translateY(-50%);border:0;background:transparent;color:#64748b;font-size:19px;cursor:pointer;padding:6px 10px}.search-select-menu{position:absolute;z-index:1500;left:0;right:0;top:calc(100% + 5px);max-height:260px;overflow:auto;background:#fff;border:1px solid #cbd5e1;border-radius:10px;box-shadow:0 16px 36px rgba(15,23,42,.18);padding:5px}.search-select-option{display:block;width:100%;text-align:left;border:0;background:#fff;color:#1e293b;padding:9px 11px;border-radius:7px;cursor:pointer;font:inherit}.search-select-option:hover,.search-select-option.active{background:#eaf3ff}.search-select-option mark{background:#fde68a;color:#78350f;border-radius:3px;padding:0 2px;font-weight:800}.search-select-empty{padding:13px;color:#64748b;font-size:13px}.empty-option{color:#64748b;border-bottom:1px solid #e2e8f0;border-radius:0;margin-bottom:4px}
  `]
})
