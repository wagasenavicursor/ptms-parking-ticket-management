...
export type SearchSelectOption = { value: string; label: string; search?: string; badge?: string; warning?: boolean; warningLabel?: string; disabled?: boolean };
  ...
  template: `
    <div class="search-select" [class.open]="open" [class.has-query]="query.trim()" [class.no-toggle]="!showToggle">
      ...
      <div class="search-select-menu" *ngIf="open">
        ...
        <button type="button" class="search-select-option" [class.active]="i===activeIndex" [class.warning-option]="option.warning" [disabled]="option.disabled" *ngFor="let option of filteredOptions; let i=index" (mousedown)="$event.preventDefault();choose(option)">
